...
- 既定Taggerモデル: `data/models/wd14`
- `engines/sd-scripts` を初期エンジンとして登録
- Python、Node、pnpm、Cargo、Git、nvidia-smi、GPU情報、内部エンジンを検証
...
実学習とサンプル生成は、同梱エンジン、ベースモデル、VAE、text encoder、dataset、GPU/CUDA/Torch環境に依存する。GUI側では起動コマンド生成と事前エラー表示までを担当し、実行ログはprocess registry経由で表示する。
